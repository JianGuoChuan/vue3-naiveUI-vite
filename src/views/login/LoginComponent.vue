	...
	<n-el>
			...
			<div class="right">
				<div class="form-wrapper">
					...
					<div class="item-wrapper">
						<n-input v-model:value="username" placeholder="请输入用户名/手机号" prefix-icon="el-icon-user"
							clearable />
					</div>
					<div class="mt-4 item-wrapper">
						<n-input v-model:value="password" placeholder="请输入密码" type="password" clearable
		...
		<div v-else class="m-login-container">
			...
			<div class="content">
				...
				<n-input class="mt-10" round placeholder="请输入密码" size="large" v-model:value="password" type="password"
					show-password-toggle :maxlength="8">
					<template #prefix>
						<n-icon>
							<PasswordIcon />
						</n-icon>
				...
				post({
					...
					.then(({
						...
						userStore.saveUser(data as UserState).then(() => {
							router
								.replace({
									path: route.query.redirect ? (route.query
										.redirect as string) : '/',
								})
								.then(() => {
									loading.value = false
